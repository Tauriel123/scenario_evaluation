...
def write_answerSheet(root_path, answer_path, output_path, scenarios_database):
    '''
    ...
    output：标准答题卡
    '''
    highD_meta = scenarios_database + "/data_for_evaluator/recording.csv"
    ...
    if int(highD_index) < 10:
        tracksMeta_path = os.path.join(scenarios_database + "/data_for_evaluator/highD-dataset-v1.0/data/0" + str(highD_index) + "_tracksMeta.csv")
    else:
        tracksMeta_path = os.path.join(scenarios_database + "/data_for_evaluator/highD-dataset-v1.0/data/" + str(highD_index) + "_tracksMeta.csv")
    ...
    for group_id, df_rows in grouped:
        if count == 0:  # ego，替换为参赛者轨迹
            ego_id = group_id
            height = df_tracksMeta[df_tracksMeta['id'] == ego_id]['height'].values.tolist()[
                0]  # 车宽
            ego_frame = df_rows['frame'].values.tolist()
            df_base = df_rows[['frame', 'id']]
            df_scenario = df_rows.drop(
                df.columns[[2, 3, 10, 11, 12, 13, 14, 15, 16, 17]], axis=1)
            # 判断主车车道
            ...
            # 深拷贝，使得复制的df拥有自己的数据与索引，而非与df_base共用
            df_nonego = df_base.copy(deep=True)
    ...
    preced = []
    preced_xv, preced_x, preced_y, preced_yv, preced_xa, preced_ya = [
        [] for _ in range(6)]
    ego_x = []
    ego_laneid = []
    ego_frame = []
...
def Evaluation(list_HAV, list_origin, reward, minTTC=2.7, area=20, safety=50, efficiency=30, comfort=20,
               dt=0.04,
               Pcomfort=0.1):
    # 调整参数预设值
    ...
    if np.shape(list_origin)[0] == 0:
        ...
        time_cost = np.shape(List_origin)[0] * dt  # time_id

        reward = reward[:np.shape(List_origin)[0]]
...
def DataSource(root_path, scenario_name):
    filename = root_path + '\index.csv'
    dataframe1 = pd.read_csv(filename, header=None, encoding='gbk')
...
def write_answerSheet_NDS(Output_file, Origin_file):
    return


def Standardization_NDS(Origin_file):
    dataframe_origin = pd.read_csv(Origin_file, header=None)
    ...
    for frame in index[1:]:
        ...
        if frame == 0:
            ...
            output.append((output[0, 2] - scenario_last[0, 2]) / 0.1)
            output.append((output[0, 3] - scenario_last[0, 3]) / 0.3)

    return


class Evaluator:
    def __init__(self,result_path,scenario_database):
    ...
    def evaluate(self):
        ...
        for root, dirs, files in os.walk(self.result_path):
            for file in files:
                ...
                scenario = file[:-9]
                # 原始轨迹数据集
                source = DataSource(self.scenario_database, scenario)
                file_path = os.path.join(self.result_path, file)
                df_reward = pd.read_csv(file_path)
                reward = np.array(df_reward)[:, -4]
                # NDS部分还没有完成
                if source == 'NDS':
                    rootpath = self.scenario_database + '/Shanghai_NDS/scenarios_for_evaluation'
                    t = 0.1
                    origin_path = os.path.join(rootpath, scenario)
                    files = os.listdir(origin_path)
                    output_file = scenario + '_exam.xosc.csv'
                    answer_path = os.path.join(self.result_path, output_file)
                    output_path = os.path.join(origin_path, 'answerSheet.csv')
                    if ~('answerSheet.csv' in files):
                        write_answerSheet_NDS(
                            origin_path, answer_path, output_path)
                else:
                    rootpath = self.scenario_database + '/highD/scenarios_for_evaluation'
                    t = 0.04
                    origin_path = os.path.join(rootpath, scenario)[:-5]
                    files = os.listdir(origin_path)
                    output_file = scenario + '.xosc.csv'
                    answer_path = os.path.join(self.result_path, output_file)
                    answer_path_test = os.path.join(
                        origin_path, scenario[:-5]+'_test.csv')
                    output_path = os.path.join(origin_path, 'answerSheet.csv')
                    if ~('answerSheet.csv' in files):
                        write_answerSheet(origin_path, answer_path, output_path, self.scenario_database)

                dataframe1 = pd.read_csv(answer_path_test, header=None)
                dataframe2 = pd.read_csv(output_path, header=None)
                list_HAV = dataframe2.values
                list_origin = dataframe1.values
                list_HAV = np.array(list_HAV[1:])
                list_origin = np.array(list_origin[1:])
                list_HAV = list_HAV.astype(np.float)
                list_origin = list_origin.astype(np.float)
                id = list_origin[0, 1]
                list_origin = list_origin[list_origin[:, 1] == id, :]
                score = Evaluation(list_HAV, list_origin, reward, minTTC=2.7, area=0, safety=50, efficiency=30, comfort=20,
                                dt=0.04,
                                Pcomfort=0.1)
                # print('score', score)
                all_score.append(score)
                # dataframe1 = pd.read_csv(output_path, header=None)
        # print(all_score)
        return np.array(all_score)

if __name__ == '__main__':
    eva = Evaluator("./outputs","../../scenarios_database")
    score = eva.evaluate()
    print(score.mean()) # 原来均分 71.50 新的均分59.67
