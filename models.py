...
class TAE_encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        out_cnn = out_cnn.permute((0, 2, 1))  # 矩阵结构变换，（0，1，2）->(0,2,1)
        ...
        features = torch.sum(  # sum求和,dim表示求和维度
            features.view(  # torch.view等同于np.reshape
...
class TAE(nn.Module):
    ...
    """
    """
    forward必须重写，但执行时基于__call__,forward会被底层自动启动
    """
    ...
    def forward(self, x):
        ...
        return features.squeeze(2), out_deconv  # features是降维后的特征
...
class ClusterNet(nn.Module):
    ...
    def init_centroids(self, x):
        """
        This function initializes centroids with agglomerative clustering 层次聚类
        + complete linkage.比较元素对间的最大距离
        """
        z, _ = self.tae(x.squeeze().unsqueeze(1).detach())
        z_np = z.detach().cpu()  # 将数据移交到cpu上，返回tensor
        ...
        )  # fit_predict:训练模型并预测每个样本的簇标记,fit:训练样本。assignements是预测的簇标签
    ...
    def forward(self, x):
        z, x_reconstr = self.tae(x)  # features.squeeze(2), out_deconv
        z_np = z.detach().cpu()
        ...
        ## P : ground truth distribution,真值的分布，训练集对监督学习技术的分类的准确性
...
class DbscanNet(nn.Module):
    ...
    def DBscan_main(self, x):
        ...
        #     compute_similarity(z_np, z_np, similarity=self.similarity)
        # )  # fit_predict:训练模型并预测每个样本的簇标记,fit:训练样本。assignements是预测的簇标签
        self.n_clusters_ = np.unique(assignments)
        self.n_clusters = len(self.n_clusters_)
        return assignments
    ...
    def init_centroids(self, x):
        ...
        for cluster_ in range(self.n_clusters):
            index_cluster = [
                k for k, index in enumerate(assignments) if index == self.n_clusters_[cluster_]
            ]
            centroids_[cluster_] = torch.mean(z.detach()[index_cluster], dim=0)
    ...
    def forward(self, x):
        ...
        similarity = compute_similarity(
            z_np, self.centroids, similarity=self.similarity
        )
